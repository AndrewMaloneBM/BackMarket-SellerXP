...
<template>
  <div class="max-w-6xl">
    ...
    <div :class="['grid gap-6 mb-6', showDeferredSummary === false ? 'grid-cols-2' : 'grid-cols-3']">
      ...
      <div class="bg-white rounded-bm border border-bm-border shadow-sm p-6">
        <div class="flex items-center justify-between gap-3"><p class="text-xs text-bm-text-mid">Next payout</p><span class="text-[11px] font-semibold rounded-full bg-blue-50 text-blue-700 px-2 py-1">Scheduled</span></div>
        <p class="text-2xl font-bold text-bm-text-hi mt-2">€13,700</p>
        <p class="text-xs text-bm-text-low mt-2">12 Aug 2026</p>
      </div>
      <div v-if="showDeferredSummary !== false" class="bg-white rounded-bm border border-bm-border shadow-sm p-6">
        <div class="flex items-center justify-between gap-3"><p class="text-xs text-bm-text-mid">Deferred payout</p><span :class="['text-[11px] font-semibold rounded-full px-2 py-1', tierBadgeClass(seller.currentTier)]">{{ tierLabel(seller.currentTier) }}</span></div>
    ...
    <div v-if="showFasterBanner !== false" class="rounded-bm border border-blue-200 bg-blue-50 p-5 mb-6 flex items-center justify-between gap-6">
      ...
      <button type="button" class="prototype-hotspot whitespace-nowrap rounded-full bg-[#0F1117] text-white text-sm font-semibold px-4 py-2 hover:bg-gray-800">Learn more</button>
    </div>

    <div class="bg-white rounded-bm border border-bm-border shadow-sm p-6 mb-8">
      ...
      <div class="overflow-hidden rounded-bm-sm border border-bm-border">
        <div class="grid grid-cols-[1.2fr_1.1fr_1fr_0.8fr] gap-4 bg-[#F7F7F9] px-4 py-3 text-xs font-semibold text-bm-text-mid"><span>Payout date</span><span>Sales period</span><span>Amount</span><span>Status</span></div>
