...
const optimiseByMarket = ref<Record<string, boolean>>({
  AT: true, BE: true, FI: true, FR: true, DE: true,
  GR: true, IE: true, IT: true, NL: true, PT: true,
  SK: true, ES: true, SE: true,
})
...
function toggleOptimise(code: string) {
  optimiseByMarket.value[code] = !optimiseByMarket.value[code]
}
...
function heldSideFor(row: PricingRow, optimiseOn: boolean): HeldSide {
  if (!optimiseOn) return 'backbox'
  if (row.backBox.status === 'opportunity') return 'backbox'
  if (row.strategy.type === 'none') return 'backbox'
  if (row.bbHigher) return 'backbox'
  return 'promo'
}

function heldReasonFor(row: PricingRow, optimiseOn: boolean): string {
  if (!optimiseOn) return 'BackBox-chasing (default behaviour).'
  if (row.backBox.status === 'opportunity') return 'Winning the BackBox first, within your range.'
  if (row.strategy.type === 'none') return 'Only option available, within your range.'
  if (row.bbHigher) return 'Earns more than the promotion here, within your range.'
  return 'Earns more than winning the BackBox here, within your range.'
}
...
function rangeText(row: PricingRow): string {
  const minR = Math.round(row.minPrice)
  const maxR = Math.round(row.targetPrice)
  if (row.currency === 'SEK') return `${minR.toLocaleString('en-US')}–${maxR.toLocaleString('en-US')} SEK`
  return `€${minR}–€${maxR}`
}
</script>
      ...
      <form class="md:flex md:items-start">
        <div class="grow">
          <div class="mb-4 grid grid-cols-2 gap-4 lg:grid-cols-5">
            <div class="relative">
              <input id="f-title-c3" type="text" placeholder=" " class="peer h-12 w-full px-3 pt-1 rounded-bm-sm border border-bm-border-action bg-white text-base text-bm-text-hi placeholder:text-transparent focus:outline-none focus:border-bm-text-hi" />
              <label for="f-title-c3" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 truncate text-base text-bm-text-hi transition-all
            ...
            <div class="relative">
              <input id="f-sku-c3" type="text" placeholder=" " class="peer h-12 w-full px-3 pt-1 rounded-bm-sm border border-bm-border-action bg-white text-base text-bm-text-hi placeholder:text-transparent focus:outline-none focus:border-bm-text-hi" />
              <label for="f-sku-c3" class="pointer-events-none absolute left-3 top-1/2 -translate-y-1/2 truncate text-base text-bm-text-hi transition-all
                                        peer-focus:top-2 peer-focus:translate-y-0 peer-focus:text-xs peer-focus:text-bm-text-low
                                        peer-[:not(:placeholder-shown)]:top-2 peer-[:not(:placeholder-shown)]:translate-y-0
                          ...
                          <td class="px-4 py-6 first:pl-6 align-middle">
                            <div class="flex flex-col gap-2">
                              ...
                              <label class="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" class="peer sr-only" :checked="optimiseByMarket[row.code]" @change="toggleOptimise(row.code)" />
                                <span :class="['relative flex h-4 w-7 shrink-0 items-center rounded-full transition-colors', optimiseByMarket[row.code] ? 'bg-bm-success' : 'bg-bm-gray-300']">
                                  <span :class="['absolute w-3 h-3 rounded-full bg-white shadow transition-all', optimiseByMarket[row.code] ? 'left-[14px]' : 'left-0.5']" />
                                </span>
                                <span class="text-[11px] font-medium text-bm-text-hi">Optimise for earnings</span>
                              </label>
                              <p class="text-[11px] text-bm-text-low">Always within your {{ rangeText(row) }} range</p>
                            </div>
                          </td>
                            ...
                            <div v-if="row.backBox.status === 'won'" class="flex items-start gap-3" :style="{ color: 'hsl(156, 100%, 21%)' }">
                              ...
                              <div class="flex flex-col gap-0.5">
                                <div class="flex items-center gap-2 flex-wrap">
                                  ...
                                  <span v-if="heldSideFor(row, optimiseByMarket[row.code]) === 'backbox'" class="inline-flex items-center gap-1 text-xs font-medium" :style="{ color: 'hsl(156, 100%, 21%)' }">
                                    ...
                                    BackPricer is holding this
                                  </span>
                                </div>
                                <p class="text-base font-semibold text-bm-text-hi">{{ fmtMoney(row.backBox.price, row.currency) }}</p>
                                <p class="mt-1 text-xs font-medium text-bm-text-low">
                                  {{ fmtMargin(row.bbMargin, row.currency) }}/unit after BM fees
                                </p>
                                <p v-if="heldSideFor(row, optimiseByMarket[row.code]) === 'backbox'" class="text-xs text-bm-text-low leading-relaxed max-w-[360px]">{{ heldReasonFor(row, optimiseByMarket[row.code]) }}</p>
                                ...
                                <button class="rounded-bm-sm px-[11px] py-[5px] mt-2 text-sm font-semibold cursor-pointer transition-colors inline-flex items-center justify-center gap-2 bg-white border border-bm-border-action text-bm-text-hi hover:bg-static-default-mid self-start">
                                  Apply price
                                </button>
                              </div>
                            </div>
                            ...
                            <div v-if="row.strategy.type === 'visibility-boost'" class="flex items-start gap-3 max-w-[400px]" :style="{ color: 'hsl(219, 27%, 40%)' }">
                              ...
                              <div class="flex flex-col gap-0.5">
                                <div class="flex items-center gap-2 flex-wrap">
                                  ...
                                  <span v-if="heldSideFor(row, optimiseByMarket[row.code]) === 'promo'" class="inline-flex items-center gap-1 text-xs font-medium" :style="{ color: 'hsl(156, 100%, 21%)' }">
                                    <svg class="w-3 h-3" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M10 1a4.5 4.5 0 0 0-4.5 4.5V9H5a2 2 0 0 0-2 2v6a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2v-6a2 2 0 0 0-2-2h-.5V5.5A4.5 4.5 0 0 0 10 1Zm3 8V5.5a3 3 0 1 0-6 0V9h6Z" clip-rule="evenodd" /></svg>
                                    BackPricer is holding this
                                  </span>
                                </div>
                                ...
                                <p v-if="heldSideFor(row, optimiseByMarket[row.code]) === 'promo'" class="text-xs text-bm-text-low leading-relaxed max-w-[360px]">{{ heldReasonFor(row, optimiseByMarket[row.code]) }}</p>
                                <button class="rounded-bm-sm px-[11px] py-[5px] mt-2 text-sm font-semibold cursor-pointer transition-colors inline-flex items-center justify-center gap-2 bg-white border border-bm-border-action text-bm-text-hi hover:bg-static-default-mid self-start">
                                  Apply price
                                </button>
                              </div>
