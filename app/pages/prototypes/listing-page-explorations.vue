                          ...
                          <table class="w-full border-collapse text-base">
                            ...
                            <tbody>
                              <tr v-for="(row, ri) in pricingFor(listing)" :key="row.code" :class="['bg-white', ri !== pricingFor(listing).length - 1 && 'border-b border-bm-border']">
                                ...
                                <td class="px-4 py-6 align-middle">
                                  <div v-if="row.backBox.status === 'won'" class="flex items-center gap-3" :style="{ color: 'hsl(156, 100%, 21%)' }">
                                    <svg class="w-6 h-6 flex-shrink-0" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2 9.19 8.63 2 9.24l5.46 4.73L5.82 21 12 17.27 18.18 21l-1.64-7.03L22 9.24l-7.19-.61L12 2z" /></svg>
                                  ...
                                  <div v-else class="flex items-center gap-3" style="color: hsl(219, 27%, 40%)">
                                    <svg class="w-6 h-6 flex-shrink-0" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="m21 8-9 4-9-4m18 0-9-4-9 4m18 0v8l-9 4m-9-12v8l9 4m0-12v12" /></svg>
                                    <div class="flex flex-col gap-0.5">
